...
@interface InsiderViewController ()<CNContactPickerDelegate,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
-(void)rightAction{
    ...
    if (self.saveInsiderBlock) {
        self.saveInsiderBlock(self.selectContactArray);
    }
    [self.navigationController popViewControllerAnimated:YES];
}
-(void)addBottomView{
    ...
    [btnAdd setTitle:@"+添加询问人" forState:UIControlStateNormal];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 1){
        ...
        cell.lblContact.text = [NSString stringWithFormat:@"询问人%lu",indexPath.section+1];
        ...
        [cell.btnTime addTarget:self action:@selector(Identity:) forControlEvents:UIControlEventTouchUpInside];
        return cell;
    }
}
-(void)Identity:(UIButton *)btn{
    ...
    [vc setSelectIdentityBlock:^(SelectList *model) {
        ...
        ContactPeopleModel *insiderModel = self.dataArray[btn.tag-100];
        insiderModel.insiderIdentity = model;
    }];
